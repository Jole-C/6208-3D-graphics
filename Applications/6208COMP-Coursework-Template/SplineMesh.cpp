...
SplineMesh::SplineMesh(LJMULevelDemo* worldptr) : MeshObject(worldptr)
{
	...
	{
		points.push_back(spline.FindPointAlongSpline(i).position);
	}
	...
	GetBody()->SetGeometry(axisMesh);
	GetBody()->SetMaterial(materialGenerator.CreateLineMaterial());
