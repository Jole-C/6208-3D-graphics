...
Spaceship::Spaceship(LJMULevelDemo* worldptr) : MeshObject(worldptr)
{
	MeshGenerator meshGenerator;
	meshPointer = meshGenerator.GenerateMeshFromOBJFile(L"spaceship.obj", Vector4f(1, 1, 1, 1));
	GetBody()->SetGeometry(meshPointer);

	texturePointer = Glyph3::RendererDX11::Get()->LoadTexture(L"wedge_p1_diff_v1.png");
	...
	GetNode()->Scale() = Glyph3::Vector3f(0.1, 0.1, 0.1);
	...
	SplinePoint point = spline.FindPointAlongSpline(0);
	GetNode()->Position() = point.position;

	time2 = timeIncrement * 2;
}
...
void Spaceship::Update()
{
	...
	SplinePoint point2 = spline.FindPointAlongSpline(time2);

	//Interpolate between the spline point and the next spline point
	GetNode()->Position() = MathHelper::LinearInterp(point.position, point2.position, moveTime);
